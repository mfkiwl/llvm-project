...
; IS__TUNIT_OPM-LABEL: define {{[^@]+}}@incdec
; IS__TUNIT_OPM-SAME: (i1 [[C:%.*]], i32* noalias nofree noundef nonnull returned align 4 dereferenceable(4) "no-capture-maybe-returned" [[V:%.*]]) #[[ATTR0:[0-9]+]] {
; IS__TUNIT_OPM-NEXT:    [[X:%.*]] = load i32, i32* [[V]], align 4
; IS__TUNIT_OPM-NEXT:    br i1 [[C]], label [[T:%.*]], label [[F:%.*]]
; IS__TUNIT_OPM:       T:
; IS__TUNIT_OPM-NEXT:    [[X1:%.*]] = add i32 [[X]], 1
; IS__TUNIT_OPM-NEXT:    store i32 [[X1]], i32* [[V]], align 4
; IS__TUNIT_OPM-NEXT:    ret i32* [[V]]
; IS__TUNIT_OPM:       F:
; IS__TUNIT_OPM-NEXT:    [[X2:%.*]] = sub i32 [[X]], 1
; IS__TUNIT_OPM-NEXT:    store i32 [[X2]], i32* [[V]], align 4
; IS__TUNIT_OPM-NEXT:    ret i32* [[V]]
;
...
; IS__TUNIT_OPM-SAME: (i1 [[C:%.*]]) #[[ATTR0]] personality i32 (...)* @__gxx_personality_v0 {
; IS__TUNIT_OPM-NEXT:    [[Q:%.*]] = alloca i32, align 4
; IS__TUNIT_OPM-NEXT:    [[W:%.*]] = call align 4 i32* @incdec(i1 [[C]], i32* noalias nofree noundef nonnull align 4 dereferenceable(4) "no-capture-maybe-returned" [[Q]]) #[[ATTR1:[0-9]+]]
; IS__TUNIT_OPM-NEXT:    [[S2:%.*]] = call { i32, i32 } @foo() #[[ATTR2:[0-9]+]]
...
; IS__TUNIT_NPM-SAME: (i1 [[C:%.*]]) #[[ATTR0]] personality i32 (...)* @__gxx_personality_v0 {
; IS__TUNIT_NPM-NEXT:    [[S2:%.*]] = call { i32, i32 } @foo() #[[ATTR1:[0-9]+]]
...
; IS__CGSCC____-NEXT:    [[X1:%.*]] = extractvalue { i32, i32 } undef, 0
; IS__CGSCC____-NEXT:    [[S2:%.*]] = call { i32, i32 } @foo() #[[ATTR1:[0-9]+]]
; IS__CGSCC____-NEXT:    br label [[OK:%.*]]
; IS__CGSCC____:       OK:
; IS__CGSCC____-NEXT:    [[X2:%.*]] = extractvalue { i32, i32 } undef, 0
; IS__CGSCC____-NEXT:    [[Z:%.*]] = add i32 undef, undef
...
; IS__TUNIT_OPM: attributes #[[ATTR2]] = { nounwind readnone }
;.
; NOT_TUNIT_OPM: attributes #[[ATTR0]] = { nofree norecurse nosync nounwind readnone willreturn }
; NOT_TUNIT_OPM: attributes #[[ATTR1:[0-9]+]] = { nounwind readnone }
;.
