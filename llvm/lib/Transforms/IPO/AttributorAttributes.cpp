...
protected:
  ...
  ChangeStatus addAccess(int64_t Offset, int64_t Size, Instruction &I,
    ...
    AAPointerInfo::Access Acc(&I, Content, Kind, Ty);
...
struct AAPointerInfoImpl
  ...
  ChangeStatus translateAndAddCalleeState(Attributor &A, const AbstractState &R,
                                          int64_t CallArgOffset, CallBase &CB) {
    return translateAndAddCalleeState(
        A, static_cast<const AA::PointerInfo::State &>(R), CallArgOffset, CB);
  }
  ...
  static bool handleLoad(Attributor &A, const AbstractAttribute &AA,
    ...
    for (Value *Obj : Objects) {
      ...
      auto CheckAccess = [&](const AAPointerInfo::Access &Acc, bool IsExact) {
        ...
        if (!Content)
          return false;
        Value *CastedContent = AA::getWithType(*Content, *AA.getAssociatedType());
        if (!CastedContent)
          return false;
        if (IsExact)
...
struct AAHeapToStackFunction final : public AAHeapToStack {
  ...
  void initialize(Attributor &A) override {
    ...
    auto CallBaseCB = [&](Instruction &I) {
      ...
      if (!IsMalloc && !IsAlignedAllocLike && !IsCalloc)
        return true;
      auto Kind = IsMalloc ? AllocationInfo::MALLOC
                           : (IsCalloc ? AllocationInfo::CALLOC
                                       : AllocationInfo::ALIGNED_ALLOC);
    ...
    bool Success = A.checkForAllCallLikeInstructions(
        CallBaseCB, *this, /* CheckBBLivenessOnly */ false,
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    for (auto &It : AllocationInfos) {
      ...
      Align Alignment(1);
      if (AI.Kind == AllocationInfo::ALIGNED_ALLOC) {
      ...
      if (AI.Kind == AllocationInfo::CALLOC) {
  ...
  Optional<APInt> getSize(Attributor &A, const AbstractAttribute &AA,
    ...
    if (AI.Kind == AllocationInfo::MALLOC)
    ...
    assert(AI.Kind == AllocationInfo::CALLOC &&
